...
namespace vmPing.Classes
{
    internal static class Util
    {
        ...
        public static void SendTestEmail(
            ...
            bool isAuthRequired,
            string username,
            // [Provável] System.Security.SecureString compila e funciona no Linux,
            // mas o .NET runtime não implementa a proteção de memória real fora do
            // Windows (a própria Microsoft marca a API como "obsolete on this
            // platform" em builds recentes). Ela ainda encripta/zera o buffer da
            // forma usual da API, só não tem a garantia adicional do SO. Mantido
            // por compatibilidade de assinatura; considerar substituir por
            // tratamento manual de char[] se isso importar para o seu caso de uso.
        ...
        // [Certo] WPF tinha MessageBox.Show síncrono e bloqueante, utilizável de
        // qualquer thread. Tentei inicialmente usar o pacote MsBox.Avalonia, mas
        // o único release publicado no NuGet (3.0.0-rc2) tem API instável demais
        // para depender — ver DialogWindow.cs para o motivo exato. Troquei para
        // UI/DialogWindow (implementação própria, sem dependência externa).
        // Isso muda sutilmente o comportamento em relação ao WPF original: quem
        // chama ShowError() não bloqueia mais esperando o usuário fechar o
        // popup — a chamada retorna imediatamente e a janela aparece de forma
        // não-modal (Show(), não ShowDialog()). Revisar os call sites na Fase 4
        // caso algum dependa da natureza bloqueante do MessageBox original.
        public static void ShowError(string message)
        {
            Dispatcher.UIThread.Post(() => DialogWindow.ErrorWindow(message).Show());
        }
